...
export default {
  ...
  mounted () {
    this.loading = true
    // 取cookie
    this.openSimple = !this.$cookie.get('fma')
    let url = Host + 'firstMathAbove'
    this.post(url, '', res => {
      this.loading = false
      if (res.status) {
        this.info = res.info
  ...
  methods: {
    ...
    click (el) {
      this.$router.push({path: '/list/' + el})
    },
    bClose () {
      this.openSimple = false
      this.$cookie.set('fma', 'Helloworld', parseInt(this.info.adInfo.interval))
    }
  }
}
</script>
